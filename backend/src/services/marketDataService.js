...
const getUsdToUzsRate = async () => {
  try {
    ...
    console.warn("[Market Fallback] Using hardcoded USD/UZS rate:", error.message);
    return {
      source: "CBU (Fallback)",
...
const getCoinGeckoTicker = async () => {
  try {
    ...
    console.warn("[Market Fallback] Using hardcoded BTC ticker:", error.message);
    return {
      symbol: "BTCUSDT",
      lastPrice: "63000.00",
      openPrice: "63500.00",
...
const getCoinGeckoKlines = async ({ symbol, interval = "1h", limit = 24 }) => {
  try {
    ...
    console.warn("[Market Fallback] Using hardcoded klines:", error.message);
    const klines = [];
    const intervalMs = parseIntervalToHours(interval) * 60 * 60 * 1000;
    const now = Date.now();
    for (let i = 0; i < limit; i++) {
      const bucketStart = now - (limit - i) * intervalMs;
      klines.push({
        openTime: bucketStart,
